  ...
  return (
    <div className="p-8 h-full bg-dark-bg dark:bg-dark-bg">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-primary-300">Expenses</h1>
            ...
            <table className="w-full text-sm hidden md:table">
              <thead>
                <tr className="bg-dark-inner dark:bg-dark-inner border-b border-primary-800">
                  <th className="text-left px-6 py-3 font-medium text-primary-300">Title</th>
                  <th className="text-left px-4 py-3 font-medium text-primary-300">Category</th>
                  <th className="text-left px-4 py-3 font-medium text-primary-300">Date</th>
                  <th className="text-left px-4 py-3 font-medium text-primary-300">Payment</th>
                ...
                {data?.items.map(exp => (
                  <tr key={exp.id} className="hover:bg-gray-800 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-medium text-primary-100">{exp.title}</p>
                      {exp.description && <p className="text-xs text-gray-500 mt-0.5 truncate max-w-xs">{exp.description}</p>}
                      {exp.notes && (
                        <p className="text-xs text-primary-400 mt-0.5 truncate max-w-xs" title={exp.notes}>
                          📝 {exp.notes}
                        </p>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-4 text-gray-300">{formatDate(exp.expense_date)}</td>
                    <td className="px-4 py-4">
                      {exp.payment_method ? (
                        <span className={`badge ${PM_COLORS[exp.payment_method] ?? 'bg-gray-700 text-gray-300'}`}>
                          {exp.payment_method.replace('_', ' ')}
                        </span>
                      ) : '—'}
                    </td>
                    <td className="px-6 py-4 text-right font-semibold text-primary-200">
              ...
              {data?.items.map(exp => (
                <div key={exp.id} className="px-4 py-4 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-medium text-primary-100 truncate">{exp.title}</p>
                    <p className="text-xs text-gray-500 mt-0.5">{formatDate(exp.expense_date)}</p>
                    {exp.category_id && (
                      <p className="text-xs text-gray-400 mt-0.5">{catMap[exp.category_id]}</p>
                    )}
                  </div>
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    <span className="font-semibold text-primary-200">{formatCurrency(exp.amount, exp.currency)}</span>
